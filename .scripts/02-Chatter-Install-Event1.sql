...
GO
                ...
                IF NOT EXISTS (SELECT * FROM sys.service_queues WHERE name = 'Chatter_ConversationQueue_Event1ChangedEvent')
	                CREATE QUEUE dbo.[Chatter_ConversationQueue_Event1ChangedEvent] WITH POISON_MESSAGE_HANDLING (STATUS = OFF)

                IF NOT EXISTS(SELECT * FROM sys.services WHERE name = 'Chatter_ConversationService_Event1ChangedEvent')
	                CREATE SERVICE [Chatter_ConversationService_Event1ChangedEvent] ON QUEUE dbo.[Chatter_ConversationQueue_Event1ChangedEvent] ([DEFAULT]) 
            

                            -- Notification Trigger check statement.
                            
                IF OBJECT_ID ('dbo.Chatter_NotificationTrigger_Event1ChangedEvent', 'TR') IS NOT NULL
                    RETURN;
                ...
                CREATE TRIGGER [Chatter_NotificationTrigger_Event1ChangedEvent]
                ON dbo.[Event1]
                ...
                IF EXISTS (SELECT * FROM sys.services WHERE name = ''Chatter_ConversationService_Event1ChangedEvent'')
                BEGIN
					...
					BEGIN
                	    ...
                	    BEGIN DIALOG @ConvHandle 
                            FROM SERVICE [Chatter_ConversationService_Event1ChangedEvent] TO SERVICE ''Chatter_ConversationService_Event1ChangedEvent'' ON CONTRACT [DEFAULT] WITH ENCRYPTION=OFF; 
