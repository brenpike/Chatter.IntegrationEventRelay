...
using Chatter.IntegrationEventRelay.Core.Tests.TestContext.Chatter.MessageBrokers;
...
namespace Chatter.IntegrationEventRelay.Core.Tests.Handlers.UsingRowChangedExecutor
{
    ...
    {
        private readonly MessageBrokerContextCreator _mbc;
        ...
        public WhenExecutingRowChange()
        {
            ...
            _mbc = Context.MessageBrokers().MessageBrokerContext;
        ...
        public async Task MustThrowIfMappingDataIsNull()
            => await Assert.ThrowsAsync<ArgumentNullException>(() => _sut.Execute(_mapper.Mock, null, _item.Mock, _mbc.Mock));

        [Fact]
        public async Task MustThrowIfSourceToIntegrationEventMapperIsNull()
            => await Assert.ThrowsAsync<ArgumentNullException>(() => _sut.Execute(null, _mappingData, _item.Mock, _mbc.Mock));

        [Fact]
        public async Task MustThrowIfEventMappingConfigItemIsNull()
            => await Assert.ThrowsAsync<ArgumentNullException>(() => _sut.Execute(_mapper.Mock, _mappingData, null, _mbc.Mock));
        ...
        {
            await _sut.Execute(_mapper.Mock, _mappingData, _item.Mock, _mbc.Mock);
        ...
        {
            _mapper.MapThrows();
            await Assert.ThrowsAnyAsync<Exception>(() => _sut.Execute(_mapper.Mock, _mappingData, _item.Mock, _mbc.Mock));
